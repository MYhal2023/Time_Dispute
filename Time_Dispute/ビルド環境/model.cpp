...
void InitLoadModel(void)
{
	...
	ENEMY *parts = GetParts();
	for (int i = 0; i < MAX_ENEMY; i++)
	{
		LoadModel(MODEL_ENEMY, &enemy[i].model);
		GetModelDiffuse(&enemy[0].model, &enemy[0].diffuse[0]);
		for (int k = 0; k < ENEMY_PARTS_NUM; k++)	//パーツが増える事を予想して、それに対応させておく
		{
			int partsNum = i * ENEMY_PARTS_NUM + k;		//パーツの添え字を求める
			LoadModel(MODEL_ENEMY_PARTS, &parts[i].model);
		}
	}
	...
	{
		LoadModel(MODEL_ROCK, &rock[i].model);
	}

	CLONE *clone = GetClone();
	for (int i = 0; i < MAX_CLONE; i++)
	{
		LoadModel(MODEL_CLONE, &clone[i].model);
		GetModelDiffuse(&clone[0].model, &clone[0].diffuse[0]);
	}
